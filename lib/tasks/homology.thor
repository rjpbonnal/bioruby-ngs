class Homology < Thor
  ...
  class Db < Homology
    ...
    def init
      if Dir.exists? "db" and Dir.exists? "conf"
        db = Bio::Ngs::Db.new("conf/homology_db.yml")
        db.create_tables("homology")
      else
  ...
  class Load < Homology
    ...
    def blast(file)      
      Bio::Ngs::Homology.blast_import(file,"conf/homology_db.yml")
    ...
    def goannotation
      Bio::Ngs::Annotation.goa_import(options[:file],"conf/homology_db.yml")
